...
# if you choose CPU backend to calculate local energy, run `make cpu`
...
Using sampling, local energy calculation and grad update parallel, just set the `use_uniq_sampling_parallel` in the YAML configuration file (e.g. `config-h2.yaml`):

```yaml
use_uniq_sampling_parallel: true
use_samples_recursive: false # ATTENTION: must set!
```
The parallel task submit as follows:
```shell
# (2) Parallel task
...
```
### Foundation / Potential Energy Surface Model Training
Set `train_dataset` in the `test/pretrain.py` and then submit with:
```shell
...
hamiltonian_type: "CPP" # hamiltonian type include local enengy: {"CPP" | "exact"}
...
```
### GPU Support
We support pytorch model infer and backward in GPU and local energy calculation speedup using GPU independently. You can set keywords in the YAML configuration file (e.g. `config-h2.yaml`):
...
The local energy calculation module is an independent software package that provides local energy calculations in quantum chemistry. It takes as input the Hamiltonian, a set of samples, and the corresponding set of wave function values, and outputs the local energy for each sample. It offers interfaces for Python, C, and Julia.

Support different configurations by different makefile targets:
- Model Precision(MP): FP32, FP64
- Hamiltonian Coefficient DataType(HCD): FP64, ComplexF64
- Backend Type(BT): CPU, GPU(Nvidia A100/H100)

Detail of makefile targets as follows:
<!-- ```
make cpu: FP32(MP) + FP64(HCD) + CPU(BT)
make gpu: FP32(MP) + FP64(HCD) + GPU(BT)
make cpu_fp64: FP64(MP) + FP64(HCD) + CPU(BT)
make gpu_fp64: FP64(MP) + FP64(HCD) + GPU(BT)
make cpu_hamCplx64: FP32(MP) + ComplexF64(HCD) + CPU(BT)
make gpu_hamCplx64: FP32(MP) + ComplexF64(HCD) + GPU(BT)
make cpu_hamCplx64_fp64: FP64(MP) + ComplexF64(HCD) + CPU(BT)
make gpu_hamCplx64_fp64: FP64(MP) + ComplexF64(HCD) + GPU(BT)
``` -->

| Make Target         | Model Precision (MP) | Hamiltonian Coefficient DataType (HCD) | Backend Type (BT) |
|----------------------|----------------------|----------------------------------------|--------------------|
| make cpu            | FP32                 | FP64                                    | CPU               |
| make gpu            | FP32                 | FP64                                    | GPU               |
| make cpu_fp64       | FP64                 | FP64                                    | CPU               |
| make gpu_fp64       | FP64                 | FP64                                    | GPU               |
| make cpu_hamCplx64  | FP32                 | ComplexF64                             | CPU               |
| make gpu_hamCplx64  | FP32                 | ComplexF64                             | GPU               |
| make cpu_hamCplx64_fp64 | FP64            | ComplexF64                             | CPU               |
| make gpu_hamCplx64_fp64 | FP64            | ComplexF64                             | GPU               |
...
```
@misc{shang2023solving,
	title = {Solving Schrodinger Equation with a Language Model},
	url = {http://arxiv.org/abs/2307.09343},
	number = {{arXiv}:2307.09343},
	publisher = {{arXiv}},
	...
	langid = {english},
	eprinttype = {arxiv},
	eprint = {2307.09343 [quant-ph]},
	keywords = {Quantum Physics},
        note = {arXiv: 2307.09343 [quant-ph]},
}
```
...
@Article{Wu2023,
    ...
    NUMBER = {6},
    ARTICLE-NUMBER = {1417},
    URL = {https://www.mdpi.com/2227-7390/11/6/1417},
    ISSN = {2227-7390},
